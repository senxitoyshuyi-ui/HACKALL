...
`bdataInfo()` (`XQSysUtil.lua:1056`) similarly runs `bdata show` and returns all factory partition key/values (serial number, country code, factory mode flags) without authentication; `fac_info` returns security-relevant device state (telnet/ssh/uart enabled flags, factory mode, SSIDs).
